   ...
   /**
    ...
    function sfSendPaygentCareer($arrData, $arrInput, $order_id, $transactionid, $pc_mobile_type, $open_id = "") {
    	...
    	if ($arrInput['career_type'] == $careerMobileTypeDocomo
    			|| (empty($arrDataMemo['memo04']) && $arrDataMemo['memo08'] == $paygentCareerAuthD)) {
    				...
    				$arrSend['career_type'] = $careerTypeDocomo;
    					...
    					|| (empty($arrDataMemo['memo04']) && $arrDataMemo['memo08'] == $paygentCareerAuthA)) {
    					...
    					// オーソリ通知URL
    					$arrSend['return_url'] = $this->app->url('homepage') . "shopping/mdl_paygent?mode=career_auth&order_id=" . $order_id . "&" . $transactionIdName . "=" . $transactionid;
    					// キャンセル通知URL
    					$arrSend['cancel_url'] = $this->app->url('homepage') . "shopping/mdl_paygent?mode=career_auth_cancel&order_id=" . $order_id . "&" . $transactionIdName . "=" . $transactionid;
    ...
    {
    	$loader = new \Twig_Loader_Filesystem(__DIR__.'/../../../../src/Eccube/Resource/template/admin/Mail');
     ...
     * @return void
     */
    public function updateOrderStatusBatch($orderId, $newStatus = null, $delFlg = null, $paymentDate = null)
    {
    	$Order = $this->app['orm.em']->getRepository('\Eccube\Entity\Order')->findOneBy(array('id' => $orderId));
		if (!is_null($Order)) {
			if (!is_null($newStatus)) {
