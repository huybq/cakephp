...
// 		$paymentService = $this->app ['eccube.plugin.service.payment'];
		...
		$CreditType = new CreditType ( $this->app, $subData, $stock, $arrData['security_code']);
	...
	function sfSendPaygentCredit($arrData, $arrInput, $order_id, $transactionid) {
		...
		// セキュリティコード
		if(isset($arrInput ['security_code'])){
		...
		if ($arrOtherParam ['credit_3d'] != 1) {
			...
			$arrSend ['term_url'] .= "shopping/mdl_paygent?mode=credit_3d&order_id=" . $order_id . "&" . $TRANSACTION_ID_NAME . "=" . $transactionid;
		}
		/**
		 * カード情報お預かり機能 *
		 */
	...
	function sfSetPaygentCreditStock($arrData, $arrInput) {
		...
		$arrSend['card_valid_term'] = $arrInput['card_month'].$arrInput['card_year'];
		// カード名義人
		$arrSend['cardholder_name'] = $arrInput['card_name01']." ".$arrInput['card_name02'];
