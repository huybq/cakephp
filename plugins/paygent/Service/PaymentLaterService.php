...
class PaymentLaterService
{
    ...
    public function laterPaymentProcess($Order)
    {
    	...
    	if ('POST' === $this->app['request']->getMethod()) {
    		try {
    			...
    			$arrData = $Order;
    			$arrInput = array();
    			$arrRet = $this->sfSendPaygentLaterPayment($arrData, $arrInput, $transactionid);

    			$sqlVal = array();
    			$sqlVal['quick_flg'] = "1";

    			$result = $this->sendData_LaterPayment($arrRet, $arrData->getPaymentTotal(), $arrData->getId(), $paygentLaterPayment, $sqlVal);
     ...
     * 戻り値：取得結果
     */
    function sfSendPaygentLaterPayment($arrData, $arrInput, $uniqid) {
     ...
     * データ送信(後払い)
     */
    function sendData_LaterPayment($arrRet, $payment_total, $orderId, $telegram_kind, $sqlVal = array()) {
    	$pluginService = $this->app['eccube.plugin.service.plugin'];
