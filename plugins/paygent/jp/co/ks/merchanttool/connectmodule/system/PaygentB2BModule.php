...
use Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\util\HttpsRequestSender;
...
use Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\util\PaygentB2BModuleLogger;
use Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\entity\ResponseDataFactory;
...
class PaygentB2BModule {
	...
	function init() {
		...
		if ($this->masterFile == null
			|| strcasecmp(get_class($this->masterFile), "Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\system\PaygentB2BModuleResources") != 0) {
			// エラーコード
			return $this->masterFile;
		}

		if ($this->logger == null
			|| strcasecmp(get_class($this->logger), "Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\util\PaygentB2BModuleLogger") != 0) {
			// エラーコード
			return $this->logger;
		}
	...
	function post() {
		...
		$PaygentB2BModule__TELEGRAM_KIND_KEY = $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_KEY'];
		...
		if ($url === false) {
			$PaygentB2BModuleConnectException__TEREGRAM_PARAM_OUTSIDE_ERROR = $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_KEY'];
		...
		$a = mb_convert_encoding($resBody, "UTF-8", 'Shift_JIS');
  		echo $a;
		// Create ResponseData
	...
	function setTelegramParamUnsetting() {
		// 接続ID
		if (!array_key_exists($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__CONNECT_ID_KEY'], $this->telegramParam)) {
			// 接続ID が未設定の場合、デフォルトID を設定
			$this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__CONNECT_ID_KEY']] = $this->defaultId;
		}

		// 接続パスワード
		if (!array_key_exists($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__CONNECT_PASSWORD_KEY'], $this->telegramParam)) {
			// 接続パスワードが未設定の場合、デフォルトパスワード を設定
			$this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__CONNECT_PASSWORD_KEY']] = $this->defaultPassword;
		}
		...
		if ($this->telegramKind != null) {
			if ($this->masterFile->isTelegramKindRef($this->telegramKind)) {
				// 決済情報照会の場合
				if (!array_key_exists($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__LIMIT_COUNT_KEY'], $this->telegramParam)) {
					// 最大検索数が未設定の場合、照会MAX件数を設定
					$this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__LIMIT_COUNT_KEY']] =
						$this->selectMaxCnt;
				}
			}
		}
	...
	function postErrorCheck() {
		...
		if (!$this->isModuleParamCheck()) {
			// モジュールパラメータエラー
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__MODULE_PARAM_REQUIRED_ERROR']
					. ": Error in indespensable HTTP request value.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__MODULE_PARAM_REQUIRED_ERROR'];
		}

		if (!$this->isTeregramParamCheck()) {
			// 電文要求パラメータエラー
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_OUTSIDE_ERROR']
					. ": HTTP request contains unexpected value.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_OUTSIDE_ERROR'];
		}

		if (!$this->isResultCSV()) {
			// 結果CSVファイル名設定エラー
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__RESPONSE_TYPE_ERROR']
					. ": CVS file name error.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__RESPONSE_TYPE_ERROR'];
		}

		if (!$this->isTeregramParamKeyNullCheck()) {
			// 電文要求Key null エラー
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR']
					. ": HTTP request key must be null.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR'];
		}
		...
		if (!$this->isTelegramParamKeyLenCheck()) {
			...
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR']
					...
					. $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KEY_LENGTH'] . " bytes.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR'];
		}
		...
		if (!$this->isTelegramParamValueLenCheck()) {
			...
			$this->resultMessage = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR']
					...
					. $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_VALUE_LENGTH'] . " bytes.";
			trigger_error($this->resultMessage, E_USER_WARNING);
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR'];
		}
	...
	function convertFileData()  {
		...
		// 取引ファイル
		if (!isset($this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__DATA_KEY']])
			...
			// ファイル内容をdataパラメータに設定
			$this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__DATA_KEY']] = $fileData;
	...
	function isTeregramParamCheck() {
		...
		// 電文種別ID エラーチェック
		if (array_key_exists($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_KEY'], $this->telegramParam)) {
			if (!StringUtil::isEmpty(
					$this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_KEY']])) {
				$rb = true;
			}
		}
	...
	function isResultCSV() {
		...
		// 結果CSVファイル名設定エラーチェック
		if (!$this->masterFile->isTelegramKindRef($this->telegramKind)
				&& $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_FILE_PAYMENT_RES'] != $this->telegramKind
				&& !StringUtil::isEmpty($this->resultCsv)) {
			$rb = false;
		} elseif ($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_FILE_PAYMENT_RES'] == $this->telegramKind
				&& StringUtil::isEmpty($this->resultCsv)) {
			$rb = false;
		}
	...
	function isTelegramParamKeyLenCheck() {
		...
		foreach($this->telegramParam as $keys => $values) {
			if (!StringUtil::isEmpty($keys)) {
				if (strlen($keys) > $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KEY_LENGTH']) {
					$rb = false;
					break;
				}
			}
	...
	function isTelegramParamValueLenCheck() {
		...
		foreach($this->telegramParam as $keys => $values) {
			// 取引ファイル内容はチェック対象外
			if ($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__DATA_KEY'] != $keys && !StringUtil::isEmpty($values)) {
				if (strlen($values) > $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_VALUE_LENGTH']) {
					$rb = false;
					break;
				}
			}
	...
	function validateTelegramLengthCheck() {
		...
		if (isset($this->telegramParam[$this->app['config']['MdlPaygent']['const']['PaygentB2BModule__DATA_KEY']])) {
			// ファイル決済
			if ($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_LENGTH_FILE'] < $telegramLength) {
				return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR'];
			}
		} else {
			// ファイル決済以外
			if ($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_LENGTH'] < $telegramLength) {
				return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__TEREGRAM_PARAM_REQUIRED_ERROR'];
			}
		}
	...
	function isCSVOutput() {
		...
		if ($this->masterFile->isTelegramKindRef($this->telegramKind)
			...
			if ($this->getResultStatus() == $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__RESULT_STATUS_ERROR']) {
				// 処理結果が異常の場合
				if ($this->getResponseCode() == $this->app['config']['MdlPaygent']['const']['PaygentB2BModule__RESPONSE_CODE_9003']) {
					// レスポンスコードが 9003 の場合
					$rb = true;
				}
			} else {
	...
	function isFilePaymentOutput() {
		if ($this->app['config']['MdlPaygent']['const']['PaygentB2BModule__TELEGRAM_KIND_FILE_PAYMENT_RES'] == $this->telegramKind
				&& !StringUtil::isEmpty($this->resultCsv)) {
			return true;
		}
		return false;
