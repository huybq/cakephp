 ...
 * All rights reserved.
 */
namespace Plugin\MdlPaygent\jp\co\ks\merchanttool\connectmodule\util;
...
class HttpsRequestSender {
	...
	public function __construct(\Eccube\Application $app, $url)
	{
		$this->app = $app;
...
//			$this->outputDebugLog("param: " . $key . " = \"" . $value . "\"");
			...
			if ($debugLogFlg and in_array($key, $this->MASK_COLUMNS) and !StringUtil::isEmpty($value)) {
				...
				$value = str_repeat($this->app['config']['MdlPaygent']['const']['HttpsRequestSender__MASK_STRING'], strlen($value) - 1) . substr($value, -1);
			}
	...
	function send($formData, $debugFlg) {
		...
		$header = array();
		$header[] = $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CONTENT_TYPE'];
		$header[] = $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP_ENCODING'];
		$header[] = $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CONTENT_LENGTH'] . ": "
			. (StringUtil::isEmpty($query)? "0" : strlen($query));
		$header[] = $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__USER_AGENT'] . ": " . "curl_php";
	...
	function procError() {
		...
		if ($errorNo <= $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_COULDNT_CONNECT']) { // 7
			// 接続問題
			$retCode = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__KS_CONNECT_ERROR'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_COULDNT_CONNECT']) { // 7
			// 接続問題
			$retCode = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__KS_CONNECT_ERROR'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_SSL_CERTPROBLEM']) {
			// 認証問題
			$retCode = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__CERTIFICATE_ERROR'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_SSL_CACERT']) {
			// 認証問題
			$retCode = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__CERTIFICATE_ERROR'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_SSL_CACERT_BADFILE']) {	// CURLE_SSL_CACERT_BADFILE
			// 認証問題
			$retCode = $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__CERTIFICATE_ERROR'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CURLE_HTTP_RETURNED_ERROR']) {
	...
	function parseResponse($data) {
		...
		// レスポンス受信
		$line = null;
		$retCode = $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP_STATUS_INIT_VALUE'];
		$bHeaderOver = false;
		$resBodyStart = 0;
		$lines = mb_split($this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CRLF'], $data);
		// ヘッダまでを読み込む
		foreach($lines as $i => $line) {
			...
			$resBodyStart += strlen($line) + strlen($this->app['config']['MdlPaygent']['const']['HttpsRequestSender__CRLF']);

			if ($retCode === $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP_STATUS_INIT_VALUE']) {
				// ステータスの解析
	...
	function parseStatusLine($line) {
		...
		if (strpos($statusLine[0], $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP'] . "/") != 0
				|| !StringUtil::isNumericLength($statusLine[1], $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__REGEXPSTATUS_LEN'])) {

			// 不正なステータスコードを受け取った
			return $this->app['config']['MdlPaygent']['const']['PaygentB2BModuleConnectException__KS_CONNECT_ERROR'];
		}

		if (!(($this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP_SUCCESS'] <= $this->statusCode)
			&& ($this->statusCode <= $this->app['config']['MdlPaygent']['const']['HttpsRequestSender__HTTP_PARTIAL_CONTENT']))) {
