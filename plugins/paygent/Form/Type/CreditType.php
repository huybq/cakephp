...
class CreditType extends AbstractType {
    ...
    public function __construct(\Eccube\Application $app, $subData = null, $stock = null, $security_code = null) {
        $this->app = $app;
    ...
    public function buildForm(FormBuilderInterface $builder, array $options) {
    	if(isset($this->stock) && $this->stock == 1){
    		$builder = $this->getFormStockChecked($builder);
    	}else{
	...
	//when stock checked
	public function getFormStockChecked(FormBuilderInterface $builder) {
		$year = $this->getZeroYear ( date ( 'Y' ), date ( 'Y' ) + 15 );
		$month = $this->getZeroMonth ();
		$this->app ['request']->request->all ();
		$Order = $this->app ['eccube.repository.order']->findOneBy ( array (
